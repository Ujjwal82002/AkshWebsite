...
const Card = ({ title, paragraph }) => {
  return (
    <div className="max-w-xs sm:max-w-sm gap-2 lg:w-72 lg:h-56 w-full h-56 overflow-hidden bg-[#b0b0ba] rounded-lg shadow-md transition-transform duration-300 ease-in-out cursor-pointer hover:scale-105 hover:shadow-xl">
      <div className="flex flex-col justify-center p-4 sm:p-6 lg:p-20">
        <h2 className="mb-2 text-xl font-display mt-[-10px] sm:mt-[-20px] lg:mt-[-55px] lg:ml-[-50px]">
          {title}
        </h2>
        <p className="text-gray-800 lg:ml-[-50px]">{paragraph}</p>
      </div>
    </div>
  );
};
